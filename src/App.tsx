  ...
  const filteredData = useMemo(() => {
    ...
    mappedResult.sort((a, b) => {
      ...
      if (sortBy === 'similarity') {
        const maxA = a.displayMatches?.length ? Math.max(...a.displayMatches.map(m => m.similarity)) : 0;
        const maxB = b.displayMatches?.length ? Math.max(...b.displayMatches.map(m => m.similarity)) : 0;
        return maxB - maxA;
      }
      if (sortBy === 'ai_score') {
    ...
    return mappedResult;
  }, [data, statusFilter, celebFilter, minSimilarity, aiFilter, minAiScore, sortBy]);
    ...
    <div className="layout">
      ...
      <aside className="sidebar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
          <Instagram size={28} />
          <h2 className="title" style={{ margin: 0 }}>CelebScan Demo</h2>
        </div>

        <div className="form-group">
          <label className="form-label">
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Search size={14} /> Celebrity</span>
          </label>
          <select
            className="select"
            value={celebFilter}
          ...
          >
            ...
            {uniqueCelebs.map(c => <option key={c} value={c}>{c.replace('_', ' ')}</option>)}
          </select>
        </div>
        ...
        <div className="form-group">
          <label className="form-label">
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Bot size={14} /> AI Status</span>
          </label>
          <select className="select" value={aiFilter} onChange={e => setAiFilter(e.target.value)}>
        ...
        <div className="form-group">
          <label className="form-label">
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><SlidersHorizontal size={14} /> Status</span>
          </label>
            ...
            <div className="grid">
              {paginatedData.map((item, idx) => (
                <div key={`${item.post_id}-${idx}-${page}`} className="card">
                  <div className="card-image-wrap" style={{ padding: '0', borderBottom: '2px solid var(--border-color)', height: '360px', position: 'relative' }}>
                    <span className="status-badge" style={{ zIndex: 10 }}>
                      {item.status}
                    </span>
                    <LazyLoadImage
                      src={item.display_url || `${item.post_url}media/?size=l`}
                      alt="Post visual"
                      effect="blur"
                      threshold={300}
                    ...
                    />
                    <div className="url-block" style={{ display: 'none', padding: '20px', position: 'absolute', top: '50%', transform: 'translateY(-50%)', width: '100%', wordBreak: 'break-all' }}>
                      <strong>POST URL</strong><br />
                      <a href={item.post_url} target="_blank" rel="noopener noreferrer" className="link">{item.post_url}</a>
                    </div>
                  </div>
                  ...
                  <div className="card-content">
                    <div className="badge-row">
                      {item.ai_generated === 'yes' && (
                        <div className="badge"><Bot size={12} /> AI Gen ({(item.ai_score * 100).toFixed(0)}%)</div>
                      )}
                      {item.ai_generated === 'no' && (
                        <div className="badge"><User size={12} /> Real ({(item.ai_score * 100).toFixed(0)}%)</div>
                      )}
                      {item.ai_generated !== 'yes' && item.ai_generated !== 'no' && item.ai_generated && (
                        <div className="badge">{item.ai_generated}</div>
                      )}
                    </div>

                    <p className="caption">{item.caption || "No caption provided."}</p>

                    {item.displayMatches && item.displayMatches.length > 0 && (
                      <div className="matches-list">
                        {item.displayMatches.map((m, i) => (
                          <div key={i} className="match-item">
                            <span className="match-celeb">{m.celebrity.replace('_', ' ')}</span>
                            <span className="match-sim">{(m.similarity * 100).toFixed(1)}% match</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="card-footer">
                    ...
                    <a href={item.post_url} target="_blank" rel="noopener noreferrer" className="link display-flex" title="View on Instagram">
                      <LinkIcon size={14} />
                    </a>
                  </div>
                </div>
